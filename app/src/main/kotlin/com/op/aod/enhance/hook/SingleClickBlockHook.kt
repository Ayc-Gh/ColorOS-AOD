    ...
    private fun YukiBaseHooker.hookDoubleClickSleepSingleTap() {
        runCatching {
            DOUBLE_CLICK_LISTENER
                ...
                .hook {
                    before {
                        if (!AodConfigReader.read(MainHook.hostAppContext).blockSingleClick) return@before
                        result = false
                        AodLog.d("SINGLE_CLICK", "view-touch path blocked single tap")
                    }
                }
        }.onSuccess {
            AodLog.i("HOOK_REGISTER_DETAIL", "SingleClick DoubleClickListener registered")
        }.onFailure {
            AodLog.e("HOOK_REGISTER_DETAIL", "SingleClick DoubleClickListener unavailable", it)
        }
    }
                ...
                .hook {
                    before {
                        ...
                        if (!cfg.blockSingleClick) {
                            gate.reset()
                            AodLog.d("SINGLE_CLICK", "$label pass-through blockSingleClick=false")
                            return@before
                        }

                        val now = SystemClock.elapsedRealtime()
                        if (gate.shouldAllow(now)) {
                            AodLog.d("SINGLE_CLICK", "$label allowed double-click")
                            return@before
                        }

                        // onClick 返回 void/Unit；before 中设置 result=null 可跳过原方法。
                        result = null
                        AodLog.d("SINGLE_CLICK", "$label blocked first/single click")
                    }
                }
        }.onSuccess {
            AodLog.i("HOOK_REGISTER_DETAIL", "SingleClick $label onClick registered")
        }.onFailure {
            AodLog.e("HOOK_REGISTER_DETAIL", "SingleClick $label onClick unavailable", it)
        }
    }
}
